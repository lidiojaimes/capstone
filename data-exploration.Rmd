...
stroke |> 
  ...
  filter(date_cov19 == "before covid",
         covid_19 == "Yes (Once)") |> 
  select(id_number,date_of_stroke,covid_19,covid_19_notes)
...
```{r Figure 4}
stroke |> 
  group_by(location_of_stroke,diabetic_neuropathy) |> 
  summarise(median_nihss=median(admission_nihss,na.rm=TRUE)) |> 
  filter(!is.na(diabetic_neuropathy)) |> 
  ggplot(aes(x = (location_of_stroke),
             y = median_nihss,
             fill = diabetic_neuropathy)) + 
  geom_bar(stat = "identity", position = "dodge", width = 0.7, color="black") +
  scale_fill_manual(name = "Diabetic Neuropathy",
                     labels = c("No","Yes"),
                     values = c("red","blue")) + 
  theme_bw() +
...
diabetes mellitus (dm) is an indicator for diabetes
