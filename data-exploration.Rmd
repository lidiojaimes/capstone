...
**Need to go in and make 532 into other and 223 as native hawaiian**

```{r}
table(stroke$date_of_stroke,useNA = "ifany")
stroke |> select(id_number,covid_19,covid_19_notes)

class(stroke$date_of_stroke)
...
```{r}
stroke |> 
  ggplot(aes(x=admission_nihss,y=location_of_stroke,fill=location_of_stroke)) + 
  geom_boxplot() + 
  theme_bw() +
